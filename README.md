...
- **Android 手机端**（`src/MacroClicker.Mobile`）：**完整连续动作录制**——像平时一样操作手机，点击、滑动、长按与操作间隔自动识别成动作序列（与桌面端体验一致），一键循环执行。Kotlin + Material 3 底栏四页 UI（宏 / 录制 / 执行 / 设置）。手势注入经 **Shizuku（ADB shell 权限）**完成——**完全不使用无障碍权限、不读取屏幕内容**。
...
- **完整动作录制**：点「开始录制」后直接操作手机即可——**点击 / 滑动 / 长按 / 等待（操作间隔）** 自动识别成事件序列，无需逐个动作添加；也可在「宏」页手动添加事件再编辑
- **回放同步**（默认开）：录制时每个动作会经 ADB shell 同步注入真实应用，界面真实响应，可以录「点按钮 → 等弹窗 → 再点」的多步流程；Shizuku 未就绪时自动转为纯标记录制
- **多宏管理**：底栏「宏」页切换（单选列表）、新建 / 重命名 / 复制 / 删除，事件可编辑（坐标/时长/间隔）、上移下移
- **循环执行**：一次 / 指定次数 / 无限循环，轮次间隔 + 开始倒计时；注入失败（Shizuku 断连）立即停止，绝不盲点
- **悬浮球**：贴边可拖动、位置记忆；展开面板即可 录制 / 执行 / 停止，录制中点球即完成保存；可在设置页选择是否常驻
- **全新 Material 3 UI**：底栏四页（宏 / 录制 / 执行 / 设置），动态取色（Android 12+）、深浅色跟随系统、边到边适配小屏
- **宏互通**：与桌面端宏 JSON 同构（`mouse_click→tap`，`swipe`/长按/`wait` 直接收）；宏内记录分辨率，跨机型导入自动按比例换算

### 安装使用

1. 下载并安装两个 APK：
   - **宏连点器**：`publish/android/MacroClicker-v3.0.0-android.apk`（随仓库分发，签名固定可覆盖安装；Actions 产物同源）
...
> 安全说明：Android 出于安全不允许普通应用向其他应用注入触摸，无障碍方案存在被滥用与被风控的风险。本应用改用 **Shizuku 以 ADB shell 权限执行 `/system/bin/input tap|swipe`**——固定参数、不经 shell 解释器，不读取屏幕内容、不使用无障碍权限；授权可在 Shizuku 中随时撤销。录制采用「全屏标记层捕获 + shell 实时回放」方案，无需 root。
...
└── MacroClicker.Mobile/         # Android 手机端（Kotlin + Material 3，仅 CI 构建）
    └── app/src/main/java/com/macroclicker/mobile/
        ...
        ├── store/MacroStore.kt  # 多宏持久化 + 分辨率换算 + 界面开关
        ├── shell/               # Shizuku 注入后端（AIDL UserService + 状态机）
        ├── service/MacroService.kt    # specialUse 前台服务：悬浮球/录制/回放引擎宿主
        ├── record/GestureRecorder.kt  # 完整动作录制层 + shell 实时回放同步
        ├── overlay/FloatingBall.kt    # 悬浮球 + 控制面板
        └── ui/                   # Material 3 底栏四页主界面 / 事件列表 / 编辑对话框
```
...
- 手机端单次 `input tap/swipe` 约 0.3–0.5 秒，事件间隔建议 ≥ 0.5 秒；Shizuku 重启手机后需重新启动，未就绪时无法执行（录制可降级为纯标记模式）
- 请勿将本工具用于违反目标平台规则的场景
