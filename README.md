...
- **自动发现**：一键「检测设备」——MuMu 12 走官方 `MuMuManager.exe`（精确到实例、ADB 端口与渲染窗口句柄，16384+32×n）；雷电（5555+2n）、夜神（62001+）、逍遥（21503）、MuMu6（7555）、蓝叠/AVD/USB 经 adb 扫描发现；也可手动输入 serial（如 `127.0.0.1:5555`）
...
- **宏独立**：模拟器宏与本机宏分开存储（`macros/emulator/`），页面下拉各自管理

### 宏管理（两端同思路）

工具栏「宏」下拉框直接列出当前页面已保存的宏，**选中即载入、输入名字保存**，不再弹文件对话框，也不记忆上次打开的文件名。宏文件为可读 JSON，位于 exe 旁 `macros/windows|emulator/` 目录（旧版平铺宏首次运行自动迁移）。
