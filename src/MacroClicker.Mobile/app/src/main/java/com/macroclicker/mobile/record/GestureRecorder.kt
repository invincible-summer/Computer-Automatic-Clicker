 ...
 *
 * 回放同步（liveReplay）：每个手势录制完成后立即经无障碍服务（仅手势派发）
 * 注入真实应用（层短暂切为 FLAG_NOT_TOUCHABLE 空透），可边标记边让界面
 * 真实响应，支持「点按钮→等弹窗→再点」的多步录制；关闭或服务未就绪时
 * 为纯标记录制（手势只被记录、不作用于界面）。
 *
 * v5：注入走无障碍手势派发（毫秒级），穿透窗口极短，物理触摸被漏记的时间窗很小。
 */
class GestureRecorder(
    ...
    /**
     * 回放同步：在录制层上发生的手势不达应用，改为把等效手势派发到层下方应用。
     * 层在派发期间置 FLAG_NOT_TOUCHABLE 放行（派发事件按坐标命中下层应用），
     * 同步派发返回后恢复可触摸；派发串行执行，永不阻塞主线程。
     */
